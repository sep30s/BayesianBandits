...
Table1.2 <- Delays1 %>%
  ...
  summarize(
    ...
    AvgTaxiOut = mean(taxi_out, na.rm = TRUE),
    AvgTaxiIn = mean(taxi_in, na.rm = TRUE),
    AvgCRSArrTime = mean(crs_arr_time, na.rm = TRUE),
    AvgArrTime = mean(arr_time, na.rm = TRUE),
    AvgArrDelay = mean(arr_delay, na.rm = TRUE),
    AvgAirTime = mean(air_time, na.rm = TRUE),
    CancelledFlights = sum(cancelled, na.rm = TRUE),
    DivertedFlights = sum(diverted, na.rm = TRUE), 
    AvgCarrierDelay = mean(carrier_delay),
    AvgSecurityDelay = mean(security_delay),
    AvgWeatherDelay = mean(weather_delay),
    AvgNASDelay = mean(nas_delay),
    AvgLateAircraftDelay = mean(lateaircraft_delay),
...
print(Table1.2_combined)

Table1.2_pivoted <- Table1.2_combined %>% 
  pivot_longer(
    cols = -flight_period,
    names_to = "Statistic", 
    values_to = "Value") %>% 
...
Table1.2_pivoted %>%
  ...
  fmt_number(
    ...
    rows = Statistic == "TotalFlights",
    decimals =0) %>%
  fmt_number(
    columns = c(Morning, Afternoon, Evening, Total),
    rows = Statistic == "TotalUniqueDates",  
    decimals = 0) %>%
  fmt_number(
    columns = c(Morning, Afternoon, Evening, Total),
    rows = Statistic == "TotalUniqueOrigins",  
    decimals = 0) %>%
  fmt_number(
    columns = c(Morning, Afternoon, Evening, Total),
    rows = Statistic == "TotalUniqueDestinations",
    decimals = 0) %>%
  fmt_time(
    columns = c(Morning, Afternoon, Evening, Total),
    rows = Statistic == "AvgCRSDepTime",
    time_style = "h_m_p") %>%
  fmt_number(
    columns = c(Morning, Afternoon, Evening, Total),
    rows = Statistic == "TotalFlights",  
  ...
  fmt_number(
    columns = c(Morning, Afternoon, Evening, Total),
    rows = Statistic == "TotalCancelled",  
    decimals = 0) %>%
  fmt_number(
    columns = c(Morning, Afternoon, Evening, Total),
    rows = Statistic == "AvgArrDelay",
    decimals = 1) %>%
  fmt_number(
    columns = c(Morning, Afternoon, Evening, Total),
    rows = Statistic == "TotalFlights",
...
```{r}
#| echo: false
ggplot(Delays, aes(x=AIR_TIME)) +
  geom_histogram() +
  labs(title = "Histogram of Air Times",
       x = "Air Time", 
       y = "Frequency")+
  xlim(NA, 400)
